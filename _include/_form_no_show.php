...
$total=SM_total("SELECT * FROM skedmaker_sked WHERE code='$appointment_code' AND noshowsent='' LIMIT 1");

if($total>0){
	$result=SM_result("SELECT * FROM skedmaker_sked WHERE code='$appointment_code' AND noshowsent='' LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=="noshow"){
	...
	$noshows++;
	$noshowsent=SM_ts();
	$noShowSked=$wpdb->update("skedmaker_sked", array("noshowsent"=>$noshowsent), array("code"=>$appointment_code));
