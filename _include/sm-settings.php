...
$sm_settings_result=$wpdb->get_results("SELECT * FROM skedmaker_users", ARRAY_A);


foreach($sm_settings_result as $row){
...
//	if($check_DST==1 && $daylight_savings=="y"){$timezone=$timezone+1;}
	...
	$requireservices=SM_d($row['requireservices']);
	...
	$services_list_style=SM_d($row['services_list_style']);


	$partymax=SM_d($row['partymax']);
	
	$limited_services=SM_d($row['limited_services']);
