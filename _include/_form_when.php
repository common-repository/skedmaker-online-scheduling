...
if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=="when"){
	...
	if($errorMessage==""){
		$total=SM_total("SELECT * FROM skedmaker_sked WHERE email='$findemail' AND startdate>'$today' LIMIT 1");
		if($total>0){
			$result=SM_result("SELECT * FROM skedmaker_sked WHERE email='$findemail' AND startdate>'$today'");
...
if($op=="when" && $loginValidClient=="y"){ 
    ...
    <?php 
	$today=SM_ts();
	$total_upcoming=SM_total("SELECT * FROM skedmaker_sked WHERE usercode='$usercode' AND startdate>'$today' LIMIT 1");
	if($total_upcoming>0){
		echo "<tr><td class='pad7b2' colspan='2'><b>Below is a list of your upcoming appointments.</b></td></tr>";
		$result=SM_result("SELECT * FROM skedmaker_sked WHERE usercode='$usercode' AND startdate>'$today'");
		foreach($result as $row){
