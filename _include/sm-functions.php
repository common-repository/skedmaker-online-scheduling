...
function SM_get_service_name($service_code){
	$result=SM_result("SELECT * FROM skedmaker_services WHERE code='$service_code' LIMIT 1");
	foreach($result as $row){$service_name=SM_d($row['name']);}
	return $service_name;
}

function SM_total($query){
	global $wpdb;
...
function SM_now(){
	$result=SM_result("SELECT * FROM skedmaker_users");
	foreach($result as $row){$timezone=SM_d($row['timezone']); $daylight_savings=SM_d($row['daylight_savings']);}
...
if(!function_exists('SM_resend_validation')){function SM_resend_validation(){
	if($_GET['op']=="resend"){
		...
		$result=SM_result("SELECT * FROM skedmaker_clients WHERE code='$valid_code' LIMIT 1");
...
if($_GET['op']=="forgot"){
	global $adminemail; global $BCC; global $wpdb;
	...
	if ($_SERVER['REQUEST_METHOD']=='POST'){
		...
		if($errorMessage==""){
			...
			$result=SM_result("SELECT * FROM skedmaker_clients WHERE email='$forgotemail' LIMIT 1");
			foreach($result as $row){$userPass=SM_d($row['password']);}
...
if(!function_exists('SM_validate_account')){function SM_validate_account(){
	...
	if($_GET['op']=="validate"){
		...
		$valid_code=SM_e($_GET['vc']);
		$total=SM_total("SELECT * FROM skedmaker_clients WHERE code='$valid_code' LIMIT 1");
		if($total>0){
			$saveIt=$wpdb->update("skedmaker_clients", array("valid"=>'y'), array("code"=>$valid_code));
			if(!$saveIt){
				SM_redBox("Could not validate, try again later.", "100%", 21);
			}else{
				$result=SM_result("SELECT * FROM skedmaker_clients WHERE code='$valid_code' LIMIT 1");
				foreach($result as $row){$sess_username=SM_d($row['username']);}
...
if(!function_exists('SM_permalink')){function SM_permalink(){
	$uses_perm=SM_total("SELECT * FROM wp_options WHERE option_name='permalink_structure' AND option_value='/%postname%/' LIMIT 1");

	$uses_default=SM_total("SELECT * FROM wp_options WHERE option_name='permalink_structure' AND option_value='' LIMIT 1");

	// check if using permalinks
	if($uses_perm>0){
		$result=SM_result("SELECT post_name FROM wp_posts WHERE post_content LIKE '%wp-skedmaker%' AND post_status='publish' LIMIT 1");
		foreach($result as $row){
	...
	// check if using default
		...
		$result=SM_result("SELECT ID FROM wp_posts WHERE post_content LIKE '%wp-skedmaker%' AND post_status='publish' LIMIT 1");
		foreach($result as $row){
			$SM_ID=SM_d($row['ID']);
		...
		$SM_permalink=get_page_link($SM_ID)."?";
	}else{
		$SM_permalink=get_permalink()."?";
//		$SM_permalink="?";
	}
...
echo "<tr><td style='padding:0px 14px 0px 14px;'><div class='navRedReminders'><a href='".$smadmin.$url."' class='b2w'><img src='".$sm_btns_dir.$img."' style='border:0px; margin-right:14px; vertical-align:middle' /><b>".$text."</b></a></div></td></tr>";
}}
...
if(!function_exists('SM_get_client_name')){function SM_get_client_name($usercode){
	...
	foreach($result as $row){$username=SM_d($row['username']);}
	return $username;
}}
...
if(!function_exists('SM_get_client_email')){function SM_get_client_email($usercode){
	$result=SM_result("SELECT email FROM skedmaker_clients WHERE usercode='$usercode' LIMIT 1");
	foreach($result as $row){$email=SM_d($row['email']);}
	return $email;
}}
...
if(!function_exists('SM_grayBox')){function SM_grayBox($msg, $width,$redBoxFontSize){	
	if(wp_is_mobile() || $width=="100%"){$width="100%";}else{$width=$width."px";}
...
global $smadmin; global $b1_color; global $loginValid; global $wpdb;
...
for($num=$currentMonth;$num<=$stopMonth;$num++){
	...
	$result=SM_result("SELECT * FROM skedmaker_custom_sked WHERE year='$year_MenuMonths' AND month='$month_MenuMonths' AND day='01'");
	foreach($result as $row) {$datecode_MenuMonths=SM_d($row['datecode']);}
...
while($day_num <= $days_in_month){
	...
	$result=SM_result("SELECT * FROM skedmaker_custom_sked WHERE date='$check_custom'");
	foreach($result as $row){$datecode_Calendar=SM_d($row['datecode']); $dc_name_for_cal_rollover=SM_d($row['name']);}
	if($datecode_Calendar!=""){$datecode_to_Pass="&amp;dc=".$datecode_Calendar; $found_custom="y";}else{$datecode_to_Pass=""; $found_custom="";}

	$total_custom_found=SM_total("SELECT * FROM skedmaker_custom_sked WHERE date='$check_custom'");
	...
	$totalBlocked=SM_total("SELECT * FROM skedmaker_blockeddates WHERE timestamp='$checkBlocked'");
	...
	$total_in_range=SM_total("SELECT * FROM skedmaker_blackouts WHERE start_date<='$tsURL' && end_date>='$tsURL' LIMIT 1");
...
while($day_num <= $days_in_month){
	...
	$countIt=$wpdb->get_results("SELECT * FROM skedmaker_blackouts WHERE start_date<='$tsURL' && end_date>='$tsURL' LIMIT 1");	
	$total_in_range=count($countIt);
...
$totalBlocked=SM_total("SELECT * FROM skedmaker_blockeddates WHERE timestamp='$checkBlocked' LIMIT 1");

if($totalBlocked>0){$foundBlocked='y';}else{$foundBlocked='n';}

//======= BLACKOUTS?
$total_blackouts=SM_total("SELECT * FROM skedmaker_blackouts WHERE start_date<='$checkBlocked' AND end_date>='$checkBlocked' Limit 1");
...
$result=SM_result("SELECT * FROM skedmaker_users LIMIT 1");
foreach($result as $row){
...
$total_custom=SM_total("SELECT * FROM skedmaker_custom_sked WHERE date='$check_custom'");
if($total_custom>0){

	$result=SM_result("SELECT datecode FROM skedmaker_custom_sked WHERE date='$check_custom' LIMIT 1");
	foreach($result as $row){$datecode=SM_d($row['datecode']);}

	$result=SM_result("SELECT name FROM skedmaker_custom WHERE datecode='$datecode' AND name!='' LIMIT 1");
	foreach($result as $row) {$customName=SM_d($row['name']);}
	$islive='y';
}
$cal_width="100%";
?>
...
$getExist=SM_result("SELECT * FROM skedmaker_custom_sked WHERE date='$check_custom' AND name!=''");
foreach($getExist as $rowExist){$codeExist=SM_d($rowExist['code']);}
...
<?php 
	...
	if($total_custom>0){
		$result=SM_result("SELECT datecode FROM skedmaker_custom_sked WHERE date='$check_custom' LIMIT 1");
		foreach($result as $row){$DBdatecode=SM_d($row['datecode']);}

		$stagger=0;
		$resultDC=SM_result("SELECT * FROM skedmaker_custom_timeframes WHERE datecode='$DBdatecode' ORDER BY starthour"); // the named one is the storage for the  is the 
		foreach($resultDC as $rowDC){
...
//	echo $year."-".$month."-".$day." ".$openhour.":".$openminute."00";
	...
	$result=SM_result("SELECT $multiplerow FROM skedmaker_users");
	foreach($result as $row){$max_apts=SM_d($row[$multiplerow]);}
...
if(!function_exists('SM_create_timeframes')){function SM_create_timeframes($max_apts, $dayTS, $end_hour, $end_minute, $increment, $appointmentAvailable, $appointmentUnavailable, $timezone, $appointmentpadding, $publicschedule, $stagger){
	...
	global $loginValid; global $smadmin; global $wpdb;

	//======= Blocked?
	$total_blocked=SM_total("SELECT * FROM skedmaker_sked WHERE startdate='$dayTS' AND (usercode='Admin Blocked' OR name='Admin Blocked') Limit 1");
...
//	$check_DST=1;
	...
	//======= get number taken
	$result=SM_result("SELECT numberinparty FROM skedmaker_sked WHERE startdate='$dayTS'");
	foreach($result as $row){
...
function SM_apt_output($this_page, $dayTS, $time_start, $time_end, $dayCheck, $remaining, $total_blocked, $time_passed, $appointmentAvailable, $appointmentUnavailable, $publicschedule, $stagger){
	global $loginValid; global $sitepublic; global $publicschedule; global $timezone; global $wpdb;
	...
	$result=SM_result("SELECT * FROM skedmaker_users");
	foreach($result as $row){$publicschedule=SM_d($row['publicschedule']);}	
...
if(!function_exists('SM_client_apt')){function SM_client_apt($dayTS, $time_start, $time_end, $op){
	...
	$result=SM_result("SELECT * FROM skedmaker_sked WHERE startdate='$dayTS'");
	foreach($result as $row){
		$datecode=SM_d($row['datecode']);
		$stagger=SM_stagger($stagger);
		$name=SM_d($row['name']);
		$DBcode=SM_d($row['code']);
		$DBnumberinparty=SM_d($row['numberinparty']);
		if($DBnumberinparty==""){$DBnumberinparty=1;}
		$noshowsent=SM_d($row['noshowsent']);
		if($op=="cancel"){
			$opURL=$smadmin."&amp;op=cancel&amp;";
			$btn_img="btn_cancel16_reg.png";
			$book_word="Cancel";
		}else if($op=="noshow" && $noshowsent!=""){
			$opURL="#";
		...
		echo "<td class='nopad'  style='border-bottom:1px dotted #666;'><div class='navYellow'><a href='".$opURL."&amp;aptc=".$DBcode."&amp;ts=".$dayTS."&amp;#skedtop' span style='font-size:11px;'><img src='".$sm_btns_dir.$btn_img."' class='btn' style='margin-left:14px;'> ".$book_word.": <b>".$name."</b> party of ".$DBnumberinparty."</a></div></td><tr>";
	}	
}}
...
if(!function_exists('SM_remove_custom_from_day')){function SM_remove_custom_from_day($customskedcode, $bulkReturn, $year, $month, $day, $weekday){
	...
	if($_GET['op']=='removecustom'){
		if ($_SERVER['REQUEST_METHOD']=='POST'){
			$result=SM_result("SELECT date FROM skedmaker_custom_sked WHERE code='$customskedcode'");
			foreach($result as $row){$this_date=SM_d($row['date']);}
			if($this_date!=""){$tsURL=SM_timestamp($this_date);}
			$saveIt=$wpdb->delete("skedmaker_custom_sked", array("code"=>$customskedcode));
			if(!$saveIt){
		...
		$total=SM_total("SELECT * FROM skedmaker_custom_sked WHERE code='$customskedcode' LIMIT 1");

		$result=SM_result("SELECT datecode FROM skedmaker_custom_sked WHERE code='$customskedcode' LIMIT 1");
		foreach($result as $row){$datecode=SM_d($row['datecode']);}

		$result=SM_result("SELECT name FROM skedmaker_custom WHERE datecode='$datecode' AND name!='' LIMIT 1");
		foreach($result as $row){$custom_name=SM_d($row['name']);}
...
if(!function_exists('SM_upcoming_warning')){function SM_upcoming_warning($datecode){
	...
	global $wpdb;
	$sm_btns_dir=plugin_dir_url(dirname( __FILE__) )."/_btns/";
	$total_currently_sked_with_datecode=SM_total("SELECT * FROM skedmaker_sked WHERE datecode='$datecode'");
	...
	if($total_currently_sked_with_datecode>0){ 
		if($total_currently_sked_with_datecode==1){$apt_word="appointment";}else{$apt_word="appointments";}?>
			...
			<?php 
			$result=SM_result("SELECT * FROM skedmaker_sked WHERE datecode='$datecode'");
			foreach($result as $row){
...
if(!function_exists('SM_cancel_apt')){function SM_cancel_apt($isAdmin){
	global $sitename; global $smadmin; global $site; global $adminemail; global $b1_color; global $wpdb;
	$sm_btns_dir=plugin_dir_url(dirname( __FILE__) )."/_btns/";
	...
	if($_GET['op']=="cancel"){
		...
		$result=SM_result("SELECT * FROM skedmaker_sked WHERE code='$ac' LIMIT 1");
		foreach($result as $row){
			if(wp_is_mobile()){
		...
		$result=SM_result("SELECT adminemail FROM skedmaker_users");
		foreach($result as $row){$adminemail=SM_d($row['adminemail']);}

		if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=="cancel"){
			$errorMessage="";

			$cancelIt=$wpdb->delete("skedmaker_sked", array("code"=>$ac));
			if(!$cancelIt){
...
if(!function_exists('SM_block_unblock')){function SM_block_unblock(){
	global $smadmin; global $wpdb;
	$ts=$_GET['ts'];
	$SAVE_BLOCK='n';
	if($_GET['op']=='unblockdate'){
		$saveIt=$wpdb->delete("skedmaker_blockeddates", array("timestamp"=>$ts));
		$save_message="Date Unblocked!";
		$SAVE_BLOCK='y';
	}else if($_GET['op']=="blockdate"){
		$saveIt=$wpdb->insert("skedmaker_blockeddates", array("timestamp"=>$ts));
...
if(!function_exists('SM_menu_admin_vertical')){function SM_menu_admin_vertical(){
	...
	$total=SM_total("SELECT * FROM skedmaker_sked WHERE startdate>'$startRange' AND startdate<'$endRange' AND (usercode!='Admin Blocked' OR name!='Admin Blocked') AND email!='' AND reminder_sent='' ");

	if($total==1){$reminder_word="Reminder";}else{$reminder_word="Reminders";}
	
	echo "<table style='width:100%;'>";

	if($total>0){ // show the red send reminders
		SM_vert_menu_red($total." ".$reminder_word. " to Send!", "btn_reminders32R_reg.png", "&amp;v=reminders&amp;");
		echo "<tr><td style='text-align:center; padding-top:0px; border-bottom:1px dotted #666;'><span class='smallRed'>".$total." ".strtolower($reminder_word)." waiting to be sent for<br>".SM_dateText($startRange)."</span></td></tr>";
	}

	SM_vert_menu("List Appointments", "btn_future32_reg.png ", "&amp;v=appointments&amp;list=future&amp;");
	SM_vert_menu("Map Appointments", "btn_map32_reg.png", "&amp;v=map&amp;");
	...
	SM_vert_menu("List Registered Clients", "btn_clients32_reg.png", "&amp;v=clients&amp;");
	SM_vert_menu("List My Services", "btn_services32_reg.png", "&amp;v=services&amp;");
...
if(!function_exists('SM_uni_check')){function SM_uni_check(){
	global $smadmin; global $loginValid; global $wpdb;
	$uni=$_POST['uni'];

	$total=SM_total("SELECT uni FROM skedmaker_uni WHERE uni='$uni'");
	...
	if($total>0){
		...
		echo "<br><br>";
		echo "<a href='".$uniback."' class='sked'><< Go back to schedule</a>";
		echo "<br><br>";
		die();
		return "y";
	}else{
		$wpdb->insert("skedmaker_uni", array("uni"=>$uni));
		return "";
	}
}}
...
if(!function_exists('SM_purge_past_check')){function SM_purge_past_check(){
	if ($_GET['op']=='purge'){
		global $timezone; global $smadmin; global $wpdb;
		$sm_btns_dir=plugin_dir_url(dirname( __FILE__) )."/_btns/";
		...
		$totalPast=SM_total("SELECT * FROM skedmaker_sked WHERE startdate<'$today'  AND usercode!='Admin Blocked' ORDER BY startdate $desc");
...
$result=SM_result("SELECT * FROM skedmaker_sked WHERE startdate<'$today'  AND usercode!='Admin Blocked' ORDER BY startdate $desc");
foreach($result as $row){
	$uc=$row['usercode'];
	$name=SM_d($row['name']);
	$phone=SM_d($row['phone']);
...
if(!function_exists('SM_purge_past_confirm')){function SM_purge_past_confirm(){
	if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=='purge_confirm'){
		...
		$saveIt=$wpdb->query("DELETE FROM skedmaker_sked WHERE startdate<$today");
		if(!$saveIt){
...
//-- delete_check
//==================================================================================================
if(!function_exists('SM_delete_check')){function SM_delete_check($DBtable){
	global $smadmin;
	...
	if ($_GET['op']=='delete'){
		...
		//-- get name
		$result=SM_result("SELECT * FROM `$DBtable` WHERE code='$code' ");
		foreach($result as $row){$name=SM_d($row['name']);}
		if($name==""){foreach($result as $row){$name=SM_d($row['username']);}}
		?>
		<form id="form2" name="form2" method="post" action="<?php echo $smadmin."&v=".$_GET['v']."&op=delete_confirm&c=".$_GET['c'];?>&">
		<?php SM_redBox("<span style='font-size:21px;'>\"".$name."\" will be permanantly deleted.</span><br><br>Are you SURE you want this deleted? This action can NOT be undone.", 800, 16);?>
		<table class='cc800'><tr>
        <td style='width:400px; text-align:right;'><div class='navMenuRound' style='width:160px; text-align:center;'><a href='<?php echo $smadmin."&v=".$_GET['v'];?>'>Cancel - Don't Delete</a></div></td>
		<td class='pad7' style='width:400px; text-align:left;'><input type="submit" name="button2" id="trash" value="Yes, Delete"/></td>
		</tr></table>
		</form>
		<?php 
		SM_foot();
...
//-- delete_confirm
//==================================================================================================
if(!function_exists('SM_delete_confirm')){function SM_delete_confirm($DBtable){
	if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=='delete_confirm'){
		global $smadmin;
		global $wpdb;
		$code=SM_e($_GET['c']);
		//-- delete permanantly
		$saveIt=$wpdb->delete("$DBtable", array("code"=>$code));
		if(!$saveIt){
			SM_redBox("Could not delete. Try again later.", 800, 21);
		}else{
			SM_greenBox("Deleted!", 800, 21);
			SM_redirect($smadmin."&v=".$_GET["v"]."&", 1000);
			SM_foot();
		}
	}
}}
...
if(!function_exists('SM_foot')){function SM_foot(){
	global $loginValid;
     ...
     <table class='cc100' style='width:100%; border:0px; margin-top:14px;'><tr><td class='pad14c'><span class='smallG' style='font-weight:normal;'>Skedmaker WordPress Plugin version .98 © Copyright Skedmaker Online Scheduling</span></td></tr></table>
	<?php 
