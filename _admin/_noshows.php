...
if($_SERVER['REQUEST_METHOD']=='POST' && $_GET['v']=="noshows" && $_GET['op']=="send"){
	...
	$result=SM_result("SELECT * FROM skedmaker_clients WHERE noshow>0 AND noshowsent=''");
	foreach($result as $row){
		...
		$resultB=SM_result("SELECT * FROM skedmaker_sked WHERE usercode='$clientCode' LIMIT 1");
		foreach($resultB as $rowB){$aptDate=SM_apt(SM_d($rowB['startdate']));}
...
//		$emailIt=emailIt("check@thisemail.com", "$adminemail", "", "Please Reschedule Your Appointment", $bodyData);
		...
		if($emailIt==true){
			...
			$today=SM_ts();
			$wpdb->update("skedmaker_clients", array("noshowsent"=>$today), array("code"=>$clientCode));
			$emails_were_sent='y';
			$not_sent="";
		}else{
...
$total=SM_total("SELECT * FROM skedmaker_clients WHERE noshow>0 AND noshowsent=''");
