...
$total_dc=SM_total("SELECT * FROM skedmaker_custom WHERE datecode='$datecode'");


$total_tc=SM_total("SELECT * FROM skedmaker_custom_timeframes WHERE datecode='$datecode'");


$result=SM_result("SELECT * FROM skedmaker_custom WHERE datecode='$datecode' LIMIT 1");
...
if($op=='deltime'){
	...
	$saveIt=$wpdb->delete("skedmaker_custom_timeframes", array("timecode"=>$timecode));
...
if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=="addedit"){
	...
	if($name==""){$name="NA";} // added for WP errors 2015-12-15	


	if($today==""){$today="NA";} // added for WP errors 2015-12-15


	if($showDate==""){$showDate="NA";} // added for WP errors 2015-12-15
	...
	if($errorMessage==""){
		...
		if($_GET['tc']!=""){
			...
			$saveIt=$wpdb->update('skedmaker_custom_timeframes', array("multiple"=>$setmultiple, "starthour"=>$starthour, "startminute"=>$startminute, "endhour"=>$endhour, "endminute"=>$endminute), array("timecode"=>$timecode));
		...
		//-- NEW
			...
			$saveIt=$wpdb->insert('skedmaker_custom_timeframes', array("name"=>$name, "date"=>$today, "showDate"=>$showDate, "multiple"=>$setmultiple, "starthour"=>$starthour, "startminute"=>$startminute, "endhour"=>$endhour, "endminute"=>$endminute, "datecode"=>$datecode, "timecode"=>$timecode));
...
if($total_dc>0){ 
	...
	if($_GET['tc']!=""){
		...
		$result=SM_result("SELECT * FROM skedmaker_custom_timeframes WHERE datecode='$datecode' AND timecode='$timecode'");
...
Select the number of appointmtnts you want to make available</td>
...
if($total_tc>0){
	...
	$result=SM_result("SELECT name FROM skedmaker_custom WHERE datecode='$datecode' LIMIT 1");
...
<?php
	...
	$result=SM_result("SELECT * FROM skedmaker_custom_timeframes WHERE datecode='$datecode' ORDER BY starthour, startminute");
