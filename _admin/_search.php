...
if($search!="" && $_GET['op']=='clients'){ 
	$total=SM_total("SELECT * FROM skedmaker_clients WHERE last LIKE'%$search%' AND live='y'");
	if($total>0){
        ...
        <table class='cc100'>
    <?php 
		$result=SM_result("SELECT * FROM skedmaker_clients WHERE last LIKE'%$search%' AND live='y' ORDER BY last, first");	
		foreach($result as $row){
			...
			$total=SM_total("SELECT * FROM skedmaker_sked WHERE usercode='$DBusercode'");

			if($total>0){ 
				$apt=SM_result("SELECT * FROM skedmaker_sked WHERE usercode='$DBusercode'");
                foreach($apt as $R2){
