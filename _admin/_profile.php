...
if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['v']=='profile'){
	...
	if($_GET['op']=="contactinfo"){
		...
		if($errorMessage==""){
			$saveIt=$wpdb->update("skedmaker_users", array("sitename"=>$sitename, "phone"=>$phone, "cellphone"=>$cellphone, "fax"=>$fax, "website"=>$website), array("live"=>"y"));
	...
	if($_GET['op']=="locationinfo"){
		...
		if($errorMessage==""){
			$saveIt=$wpdb->update("skedmaker_users", array("address1"=>$address1, "address2"=>$address2, "city"=>$city, "region"=>$region, "zipcode"=>$zipcode, "country"=>$country, "directionsURL"=>$directionsURL), array("live"=>"y"));
	...
	if($_GET['op']=="description"){
		...
		$saveIt=$wpdb->update("skedmaker_users", array("content"=>$content), array("live"=>"y"));
