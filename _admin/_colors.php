...
if ($_SERVER['REQUEST_METHOD']=='POST'){
	...
	if($_GET['op']=="color"){
		$skin=$_POST['skin'];
		$saveIt=$wpdb->update('skedmaker_users', array('skin' => $skin), array('live'=>'y'));
	...
	if($_GET['op']=="customcolor"){
		...
		if($delete_codes==""){
			...
			if($errorMessage==""){
				...
				$saveIt=$wpdb->update('skedmaker_users', array('color1' => '$color1', 'color2' => $color2, 'color3' => $color3, 'highlight' => $highlight));
...
//				$saveIt=mysql_query("UPDATE skedmaker_users SET color1='$color1', color2='$color2', color3='$color3', highlight='$highlight'");
				...
				$saveIt=$wpdb->update('skedmaker_users', array('color1' => '', 'color2' => '', 'color3' => '', 'highlight' => ''));
...
$row=$wpdb->get_row("SELECT * FROM skedmaker_users", ARRAY_A);
