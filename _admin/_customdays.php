...
if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=="deleteconfirm"){
	...
	if($_GET['dc']!=""){
		...
		$deleteIt=$wpdb->delete("skedmaker_custom", array("datecode"=>$datecode));


		$clearSked=$wpdb->delete("skedmaker_custom_sked", array("datecode"=>$datecode));


		$clearTimeframes=$wpdb->delete("skedmaker_custom_timeframes", array("datecode"=>$datecode));
...
if($_GET['op']=="delete"){
	...
	$result=SM_result("SELECT name FROM skedmaker_custom WHERE datecode='$datecode' AND starthour='' LIMIT 1");


	foreach($result as $row){$DBname=SM_d($row['name']);}
...
<br />
	...
	$result=SM_result("SELECT * FROM skedmaker_custom_timeframes WHERE datecode='$datecode' AND starthour!='' ORDER BY starthour, startminute");
...
if ($_SERVER['REQUEST_METHOD']=='POST' && ($_GET['op']=="new" || $_GET['op']=="edit")){
	...
	if($_GET['c']==""){
		...
		$saveIt=$wpdb->insert('skedmaker_custom', array('name' => $name, 'datecode' => $datecode));
		...
		$saveIt=$wpdb->update('skedmaker_custom', array('name' => $name, 'datecode' => $datecode), array('datecode'=>$code));


		$wpdb->update('skedmaker_custom_sked', array('name' => $name), array('datecode'=>$code));
...
$total=SM_total("SELECT * FROM skedmaker_custom ORDER BY name");
...
<table class='cc800'><tr><td class='blueBanner1'>Your Current Custom Days</td></tr>
	...
	foreach($result as $row){
		...
		$total_timeframes=SM_total("SELECT * FROM skedmaker_custom_timeframes WHERE datecode='$DBdatecode'");
		...
		if($total_timeframes>0){
			...
			$result_START=SM_result("SELECT * FROM skedmaker_custom_timeframes WHERE datecode='$DBdatecode' ORDER BY starthour LIMIT 1");
			...
			$result_END=SM_result("SELECT * FROM skedmaker_custom_timeframes WHERE datecode='$DBdatecode' ORDER BY starthour DESC LIMIT 1");


			foreach($result_END as $row_END){$TS_END=strtotime("6/6/1975 ".SM_d($row_END['endhour']).":".SM_d($row_END['endminute']));}
...
if($_GET['c']!=""){
	...
	$result=SM_result("SELECT * FROM skedmaker_custom WHERE datecode='$code' AND name!='' ORDER BY name");
