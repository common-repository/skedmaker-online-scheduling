	...
	for($loopMonth=$month; $loopMonth<13; $loopMonth++){
		...
		while($day_num <= $days_in_month){
			...
			if($theCheckBox!=""){
				...
				$weekday_ts=SM_timestamp($customdate."01:00.00");
				$weekday=date('l', $weekday_ts);

				$saveIt=$wpdb->insert('skedmaker_custom_sked', array("datecode"=>$datecode, "date"=>$date_for_DB, "year"=>$customYear, "month"=>$customMonth, "day"=>$customDay, "weekday"=>$weekday, "name"=>$customName, "code"=>$DBcode));
...
$ch=$wpdb->get_results("SELECT * FROM skedmaker_custom");
...
<tr><td class='pad14' style='padding-bottom:0px;'><b>Then you will be directed to a list of days where you can apply this setting.</b></td></tr>
    ...
    <td style='padding:7px; background-color:#666;'><span style='font-weight:bold; color:#fff;'>Name</span></td>


    <td style='padding:7px; background-color:#666;'><span style='font-weight:bold; color:#fff;'># Timeframes</span></td>


	<td style='padding:7px; background-color:#666;'><span style='font-weight:bold; color:#fff;'>Start Time</span></td>


	<td style='padding:7px; background-color:#666;'><span style='font-weight:bold; color:#fff;'>End Time</span></td>    
	...
	$result=SM_result("SELECT * FROM skedmaker_custom ORDER BY name");
	...
	foreach($result as $row){
		...
		$total=SM_total("SELECT * FROM skedmaker_custom_timeframes WHERE datecode='$DBdatecode' AND timecode!=''");





		$result_START=SM_result("SELECT * FROM skedmaker_custom_timeframes WHERE datecode='$DBdatecode' && timecode!='' ORDER BY starthour LIMIT 1");


		foreach($result_START as $row_START){$TS_START=strtotime("6/6/1975 ".SM_d($row_START['starthour']).":".SM_d($row_START['startminute']));}





		$result_END=SM_result("SELECT * FROM skedmaker_custom_timeframes WHERE datecode='$DBdatecode' && timecode!='' ORDER BY starthour DESC LIMIT 1");


		foreach($result_END as $row_END){$TS_END=strtotime("6/6/1975 ".SM_d($row_END['starthour']).":".SM_d($row_END['startminute']));}
		...
		if($savename!=$name){
			...
			echo "<td class='list_left' style='border-left:0px; width:35%;'><div class='navMenuSM'><a href='".$smadmin."&amp;v=customapply&amp;dc=".$DBdatecode."&amp;' class='sked'><span style='font-weight:normal; color:#000;'>$name</span></a></div></td>";
...
for($loopMonth=$use_for_loop_month; $loopMonth<13; $loopMonth++){
	...
	while($day_num <= $days_in_month){
		...
		$isCustom=SM_total("SELECT * FROM skedmaker_custom_sked WHERE date='$loopDate'");
		...
		// -- ALREADY SET TO A CUSTOM
			...
			$getExist=SM_result("SELECT * FROM skedmaker_custom_sked WHERE date='$loopDate' AND name!=''");


			foreach($getExist as $rowExist){
