...
if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=='delete_confirm'){
	$code=$_GET['c'];
	$saveIt=$wpdb->delete("skedmaker_blackouts", array("code"=>$code));
	if(!$saveIt){
...
if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=="blackout"){
	...
	if($errorMessage==""){
		...
		if($_GET['c']==""){
			$code=SM_code();
			$saveIt=$wpdb->insert('skedmaker_blackouts', array('code' => $code, 'start_date' => $BO_start, 'end_date' => $BO_end));
		}else{
			$code=SM_e($_GET['c']);
			$saveIt=$wpdb->update('skedmaker_blackouts', array('start_date' => $BO_start, 'end_date' => $BO_end), array('code'=>$code));
		}
...
if ($_GET['op']=='delete'){
	...
	$result=$wpdb->get_results("SELECT * FROM skedmaker_blackouts WHERE code='$code'", ARRAY_A);
...
<?php 
$countIt=$wpdb->get_results("SELECT * FROM skedmaker_blackouts");
$total=count($countIt);
...
<?php     
	$result=$wpdb->get_results("SELECT * FROM skedmaker_blackouts ORDER by start_date", ARRAY_A);
	foreach($result as $row){
		$start=SM_dateText(SM_d($row['start_date']));
		$end=SM_dateText(SM_d($row['end_date']));
